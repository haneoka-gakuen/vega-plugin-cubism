...
export abstract class CubismRenderer {
  ...
  /**
   * 描画元がGL状態境界を明示的に管理する場合の高速描画。
   * 通常のSDK利用者はdrawModel()を使用すること。
   */
  public drawModelWithoutStatePreservation(): void {
    if (this.getModel() == null) return;
    this.doDrawModel();
  }
  ...
  public getModelColor(): CubismTextureColor {
    return this.getModelColorInto(new CubismTextureColor());
  }

  /**
   * モデルの色を指定されたカラーへコピーする。
   * 描画ループ内で一時オブジェクトを再利用するために使用する。
   *
   * @param target コピー先
   * @return コピー先
   */
  public getModelColorInto(
    target: CubismTextureColor
  ): CubismTextureColor {
    target.r = this._modelColor.r;
    target.g = this._modelColor.g;
    target.b = this._modelColor.b;
    target.a = this._modelColor.a;
    return target;
  }
