...
export class CubismEyeBlink {
  ...
  public setBlinkingInterval(blinkingInterval: number): void {
    ...
    this._blinkingIntervalMinimumSeconds = null;
    this._blinkingIntervalMaximumSeconds = null;
  }

  /** Configure a uniform interval range, matching Cubism for Unity inputs. */
  public setBlinkingIntervalRange(minimum: number, maximum: number): void {
    this._blinkingIntervalMinimumSeconds = Math.max(0, minimum);
    this._blinkingIntervalMaximumSeconds = Math.max(this._blinkingIntervalMinimumSeconds, maximum);
  }
  ...
  public updateParameters(
    ...
    blinkingPhaseSpeed = 1
  ): void {
    // CubismAutoEyeBlinkInput idling subtracts unscaled delta from the next
    // random interval. Its Timescale applies only while Closing/Closed/Opening.
    const phaseActive =
      this._blinkingState === EyeState.EyeState_Closing ||
      this._blinkingState === EyeState.EyeState_Closed ||
      this._blinkingState === EyeState.EyeState_Opening;
    this._userTimeSeconds +=
      deltaTimeSeconds * (phaseActive ? Math.max(0, blinkingPhaseSpeed) : 1);
  ...
  public determinNextBlinkingTiming(): number {
    ...
    if (
      ...
      return (
        this._userTimeSeconds +
        this._blinkingIntervalMinimumSeconds +
        r * (this._blinkingIntervalMaximumSeconds - this._blinkingIntervalMinimumSeconds)
      );
    }
    return (
  ...
  _blinkingIntervalMinimumSeconds: number | null;
  _blinkingIntervalMaximumSeconds: number | null;
