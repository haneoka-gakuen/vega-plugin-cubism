...
export class CubismModel {
  ...
  public getMultiplyColor(index: number): CubismTextureColor {
    return this.getMultiplyColorInto(index, new CubismTextureColor());
  }
   ...
   * @returns コピー先
   */
  public getMultiplyColorInto(
    index: number,
    target: CubismTextureColor
  ): CubismTextureColor {
    ...
    if (
      ...
      const color = this._userMultiplyColors.at(index).color;
      target.r = color.r;
      target.g = color.g;
      target.b = color.b;
      target.a = color.a;
    ...
    const colors: Float32Array = this._model.drawables.multiplyColors;
    const offset = index * 4;
    target.r = colors[offset];
    target.g = colors[offset + 1];
    target.b = colors[offset + 2];
    target.a = colors[offset + 3];
    return target;
  }
  ...
  public getScreenColor(index: number): CubismTextureColor {
    return this.getScreenColorInto(index, new CubismTextureColor());
  }

  /**
   * スクリーン色を指定されたカラーへコピーする。
   * @param index Drawablesのインデックス
   * @param target コピー先
   * @returns コピー先
   */
  public getScreenColorInto(
    index: number,
    target: CubismTextureColor
  ): CubismTextureColor {
    ...
    if (
      ...
      const color = this._userScreenColors.at(index).color;
      target.r = color.r;
      target.g = color.g;
      target.b = color.b;
      target.a = color.a;
    ...
    const colors: Float32Array = this._model.drawables.screenColors;
    const offset = index * 4;
    target.r = colors[offset];
    target.g = colors[offset + 1];
    target.b = colors[offset + 2];
